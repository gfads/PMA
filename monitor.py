...
def fix_metrics(metric, parameters):
    for p in parameters:
        if p[0][0] == "$":
            metric = metric.replace(p[0], p[1])
        elif p[0][-1] == "=":
            metric = metric.replace(p[0], p[1])
        else:
            metric = metric.replace("\"" + p[0] + "\"", "\"" + p[1] + "\"")
...
class Monitor:
    ...
    def monitor(self, deploys, kind='reactive'):
        from math import isnan
        collect_replicas(deploys)
        ...
        for k_deploy, deploy in deploys.items():
            for qn, query in deploy['queries'].items():
                ...
                if kind == 'reactive':
                    ...
                    if isnan(query['current']): query['current'] = 0.01
...
def collect_replicas(deploy):
    ...
    for name, deploy in deploy.items():
        namespace = deploy['namespace']

        try:
            api_response = api_instance.read_namespaced_deployment_scale(name, namespace)
            deploy['replicas']['current'] = api_response.status.replicas
